...
int main ()
{
  ...
  int port = 54545;
  int e;
	int i=1; // compte les clients (Q10)
  ...
  while(1)
  {
    ...
    //On incrémente le nombre de client (Q10)
	  i++; 
		
    // On peut maintenant écrire
	  if(fork()==0)
    {
      //Le fils travaille et le pere continue d'écouter (Q10)
      write_file(new_sock);
		  printf("[+]Data written in the text file ");
	  }
    else
	    close(new_sock); // On ferme la socket de service (Q10)
  }
}
